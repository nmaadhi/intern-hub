...
router.post('/mentors', async (req, res) => {
  try {
    ...
    if (trimmedName.length < 2 || trimmedName.length > 100) return res.status(400).json({ error: 'Name must be 2 to 100 characters' });
    ...
    if (trimmedPhone) {
      ...
      if (existingPhone) return res.status(409).json({ error: 'A user with this phone number already exists' });
    }
    const tempPassword = generateTempPassword(12);
    const hashedPassword = await hashPassword(tempPassword);
    const mentor = await prisma.user.create({
      data: {
        name: trimmedName, email: normalizedEmail, phone: trimmedPhone,
        password: hashedPassword, role: 'MENTOR', status: 'ACTIVE', mustChangePassword: true,
      },
      select: { id: true, name: true, email: true, phone: true, role: true, status: true, mustChangePassword: true, createdAt: true },
    });
    const setupToken = generateToken();
    await prisma.passwordResetToken.create({
      data: { userId: mentor.id, token: setupToken, expiresAt: new Date(Date.now() + 60 * 60 * 1000) },
    });
    let emailSent = false;
    let emailError = null;
    try {
      await sendMentorWelcome({ name: mentor.name, email: mentor.email, tempPassword, setupToken });
      emailSent = true;
    } catch (err) {
      emailError = err.message;
    }
    res.status(201).json({
      message: 'Mentor created successfully', mentor, emailSent,
...
router.patch('/mentors/:id/status', async (req, res) => {
  try {
    ...
    res.json({ message: `Mentor ${status === 'ACTIVE' ? 'activated' : 'deactivated'} successfully`, mentor: updated });
  } catch (err) {
  ...
  try {
    ...
    const cohort = await prisma.cohort.create({
      data: {
        ...
        startDate: start, endDate: end, status: 'ACTIVE', mentorId: mentorId || null,
      },
...
router.get('/cohorts', async (req, res) => {
  try {
    ...
    const formatted = cohorts.map((c) => ({
      ...
      createdAt: c.createdAt, mentor: c.mentor, internCount: c._count.interns,
    }));
...
router.patch('/cohorts/:id', async (req, res) => {
  try {
    ...
    const updated = await prisma.cohort.update({
      ...
      include: { mentor: { select: { id: true, name: true, email: true } }, _count: { select: { interns: true } } },
    });
    res.json({
...
router.post('/interns', async (req, res) => {
  try {
    ...
    const intern = await prisma.user.create({
      data: {
        name: trimmedName, email: normalizedEmail, phone: trimmedPhone,
        dob: parsedDob, college: trimmedCollege, internId,
        password: hashedPassword, role: 'INTERN', status: 'ACTIVE', mustChangePassword: true,
        cohortId: cohortId || null, mentorId: mentorId || null,
      },
      select: {
    ...
    const setupToken = generateToken();
    await prisma.passwordResetToken.create({
      data: { userId: intern.id, token: setupToken, expiresAt: new Date(Date.now() + 60 * 60 * 1000) },
    });
    let emailSent = false;
    let emailError = null;
    try {
      await sendInternWelcome({ name: intern.name, email: intern.email, internId, tempPassword, setupToken });
      emailSent = true;
    } catch (err) {
      emailError = err.message;
    }
    res.status(201).json({
      message: 'Intern created successfully', intern, emailSent,
      ...(emailSent ? {} : { tempPassword, emailError, _note: 'Email failed — share password manually' }),
    });
  } catch (err) {
...
router.patch('/interns/:id/status', async (req, res) => {
  try {
    ...
    const updated = await prisma.user.update({
      where: { id }, data: { status },
      select: { id: true, name: true, email: true, internId: true, status: true },
    });
    res.json({ message: `Intern ${status === 'ACTIVE' ? 'activated' : 'deactivated'} successfully`, intern: updated });
  } catch (err) {
    console.error('💥 Error updating intern status:', err);
    res.status(500).json({ error: 'Failed to update intern status' });
  }
});
...
router.patch('/interns/:id/cohort', async (req, res) => {
  try {
    ...
    const updated = await prisma.user.update({
      where: { id }, data: { cohortId: newCohortId },
      select: {
        id: true, name: true, internId: true,
        cohort: { select: { id: true, name: true, mentor: { select: { id: true, name: true, email: true } } } },
      },
    });
    res.json({ message: newCohortId ? 'Intern assigned to cohort' : 'Intern removed from cohort', intern: updated });
  } catch (err) {
...
router.patch('/interns/:id/mentor', async (req, res) => {
  try {
    ...
    const updated = await prisma.user.update({
      where: { id }, data: { mentorId: newMentorId },
      select: { id: true, name: true, internId: true, mentor: { select: { id: true, name: true, email: true } } },
    });
    res.json({ message: newMentorId ? 'Mentor assigned to intern' : 'Mentor removed from intern', intern: updated });
  } catch (err) {
