...
const TIPS = [
  'Be specific — "fixed the login bug" is better than "worked on code"',
  'Mention if you need help — that\'s what standups are for',
    ...
    const load = async () => {
      try {
        ...
        if (res.data.postedToday && res.data.standups.length > 0) {
          ...
          setSprintId(todayStandup.sprintId || '');
        }

        // Load active sprint for this intern's cohort
        ...
        if (cohortId) {
          ...
          setSprints(active);
          if (active.length > 0 && !sprintId) setSprintId(active[0].id);
        }
      } catch (err) {
    ...
    <div className="space-y-6">
      ...
      {postedToday ? (
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-600">Post your update below so your mentor knows what you're working on.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              🕐 What did you do <span className="text-emerald-600">yesterday</span>?
            </label>
            <textarea
              value={yesterday}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              🎯 What are you doing <span className="text-blue-600">today</span>?
            </label>
            <textarea
              value={today}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              🚫 Any <span className="text-red-500">blockers</span>? (optional)
            </label>
            <textarea
              value={blockers}
