    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <p className="text-xs text-gray-500">
              Mentor and cohort are independent — assign either, both, or neither now. Intern ID is auto-generated (INT-XXXX).
            </p>
            <button
              type="submit"
              disabled={submitting}
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
          ...
          {createResult.emailSent ? (
            <p className="text-sm text-emerald-700 mt-1">
              A welcome email was sent to {createResult.intern.email} with login credentials.
            </p>
          ) : (
            <>
              ...
              <p className="text-xs text-amber-600 mt-2">Email could not be sent. Share this password manually.</p>
            </>
          )}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        ...
        {loading ? (
          ...
          <p className="text-gray-500 text-center py-8">No interns yet. Click "+ New Intern" above.</p>
        ) : (
              ...
              <div key={i.id} className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {i.college && (
                      <p className="text-xs text-gray-500 mt-0.5">
                        {i.college}{i.dob ? ` - born ${fmtDate(i.dob)}` : ''}
                      </p>
                    )}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${i.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-600'}`}>
                      {i.status}
                    </span>
                  </div>
                </div>
                ...
                {mentorPickerFor === i.id ? (
                  <div className="bg-purple-50 rounded-lg p-3 mt-2">
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {i.mentor && (
                        <button
                          onClick={() => handleAssignMentor(i.id, null)}
                      ...
                      {activeMentors.length === 0 ? (
                        <p className="text-sm text-gray-500">No active mentors.</p>
                      ) : (
                        activeMentors.map((m) => {
                          ...
                          return (
                            <button
                              key={m.id}
                              onClick={() => handleAssignMentor(i.id, m.id)}
                              disabled={isCurrent}
                  ...
                  <button
                    onClick={() => setMentorPickerFor(i.id)}
                ...
                {cohortPickerFor === i.id ? (
                  <div className="bg-blue-50 rounded-lg p-3 mt-3">
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {i.cohort && (
                        <button
                          onClick={() => handleAssignCohort(i.id, null)}
                          className="w-full text-left text-sm px-3 py-2 rounded bg-white hover:bg-red-50 text-red-600"
                        >
                      ...
                      {activeCohorts.length === 0 ? (
                        ...
                        activeCohorts.map((c) => {
                          ...
                          return (
                            <button
                              key={c.id}
                              onClick={() => handleAssignCohort(i.id, c.id)}
                              disabled={isCurrent}
                  ...
                  <button
                    onClick={() => setCohortPickerFor(i.id)}
