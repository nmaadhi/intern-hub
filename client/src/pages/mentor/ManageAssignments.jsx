...
function ManageAssignments() {
  ...
  const [assignType, setAssignType] = useState('cohort'); // 'cohort' | 'direct'
  ...
  const resetForm = () => {
    ...
    setCohortId(''); setSelectedInternIds([]);
    setAssignType('cohort'); setCreateMessage(null);
  };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setCreateMessage({ success: true, type: assignType });
      resetForm();
      await loadAll();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <div className="flex gap-2 mb-5">
            <button type="button" onClick={() => setAssignType('cohort')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${assignType === 'cohort' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              Assign to Cohort
            </button>
            <button type="button" onClick={() => setAssignType('direct')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${assignType === 'direct' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required minLength={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Build a REST API with Express" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Description (optional)</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Explain what needs to be done..." />
            </div>
            <div>
            ...
            {assignType === 'cohort' && (
              <div>
                ...
                {activeCohorts.length === 0 ? (
                  <p className="text-sm text-gray-500 bg-gray-50 p-3 rounded-lg">You have no active cohorts. Ask an admin to assign you to one.</p>
                ) : (
            ...
            {assignType === 'direct' && (
              <div>
                ...
                {interns.length === 0 ? (
                  <p className="text-sm text-gray-500 bg-gray-50 p-3 rounded-lg">You have no directly assigned interns. Ask an admin to assign some to you.</p>
                ) : (
      ...
      <div className="space-y-3">
        {loading ? (<p className="text-gray-500 text-center py-4">Loading...</p>) : error ? (<p className="text-red-600 text-center py-4">{error}</p>) : assignments.length === 0 ? (
            ...
            return (
              <Link key={a.id} to={`/mentor/assignments/${a.id}`} className="block bg-white rounded-2xl shadow-sm p-5 hover:shadow-md transition">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${a.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-600'}`}>{a.status}</span>
                </div>
